...
webSocketServer.on('connection', client => {
        ...
        switch (action) {
            case serverEvents.incoming.READY: {
                ...
                for (let i = 0; i < clientIds.length; i++) {
                    const c = clients[clientIds[i]];
                    if (c) {
                        c.send(makeMessage(serverEvents.outgoing.INITIAL_STATE,
                            JSON.stringify({
                                clientId: clientId,
    ...
    client.onclose = () => {
        ...
        for (const key in clients) {
            if (key !== clientId) {
                clients[key].send(makeMessage(serverEvents.outgoing.REMOVE_CLIENT,
...
async function init() {
    ...
    rayCastCallback = new RayCastCallback(metaData);
...
function physicsLoop() {
    ...
    for (let i = 0; i < clientIds.length; i++) {
        const target = clients[clientIds[i]];
        if (target) {
            for (let j = 0; j < clientIds.length; j++) {
                const source = clients[clientIds[j]];
                if (source) {
                    const playerBodyPosition = clientBodies[j].GetPosition();
                    const x = playerBodyPosition.x * pixelsPerMeter;
                    const y = playerBodyPosition.y * pixelsPerMeter;
                    target.send(makeMessage(serverEvents.outgoing.CURRENT_STATE,
                        JSON.stringify({
    ...
    for (let i = 0; i < clientIds.length; i++) {
        const rb = clientBodies[i];
        const x = rb.GetPosition().x;
        const y = rb.GetPosition().y;
        // Left ray
        ...
        world.RayCast(rayCastCallback.instance, leftRayBeginPoint, leftRayEndPoint);
        // Right ray
        ...
        world.RayCast(rayCastCallback.instance, rightRayBeginPoint, rightRayEndPoint);
    }
