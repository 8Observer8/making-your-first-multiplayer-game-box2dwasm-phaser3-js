...
const wallInfo = [];
wallInfo[0] = { x: 0, y: 300, w: 10, h: 600, xOffset: -5, yOffset: 0, category: entityCategory.REST_WALLS };
wallInfo[1] = { x: 800, y: 300, w: 10, h: 600, xOffset: 5, yOffset: 0, category: entityCategory.REST_WALLS };
wallInfo[2] = { x: 400, y: 0, w: 800, h: 10, xOffset: 0, yOffset: -5, category: entityCategory.TOP_WALL };
...
async function init() {
    ...
    for (let i = 0; i < wallInfo.length; i++) {
        ...
        const filter = new b2Filter();
        filter.categoryBits = wallInfo[i].category;
        fixture.SetFilterData(filter);
    }
    ...
    for (let i = 0; i < 12; i++) {
        ...
        shape.m_radius = 10 / pixelsPerMeter;
        const bodyDef = new b2BodyDef();
        bodyDef.type = b2_dynamicBody;
        const initialPosX = starX / pixelsPerMeter;
        const initialPosY = 0;
        bodyDef.set_position(new b2Vec2(initialPosX, initialPosY));
        const body = world.CreateBody(bodyDef);
        body.SetFixedRotation(true);
        starBodies.push(body);
        const fixture = body.CreateFixture(shape, 1);
        fixture.SetRestitution(randomFloat(0.4, 0.8));
        starFixtures.push(fixture);

        const starId = shortId.generate();
        starInfo.push({ id: starId, visible: true, position: { x: initialPosX, y: initialPosY } });
        metaData[getPointer(fixture)] = {
            id: starId,
            type: 'star',
    ...
    const contactListener = new ContactListener(world, pixelsPerMeter, metaData,
        clientIds, players, starInfo, starFixtures, bombInfo, bombBodies);
...
function physicsLoop() {
    ...
    for (let i = 0; i < clientIds.length; i++) {
        ...
        p.socket.send(makeMessage(serverEvents.outgoing.STAR_INFO,
            JSON.stringify({ starInfo: starInfo })));

        p.socket.send(makeMessage(serverEvents.outgoing.BOMB_INFO,
            JSON.stringify({ bombInfo: bombInfo })));
    }

    for (let i = 0; i < starBodies.length; i++) {
        const body = starBodies[i];
        starInfo[i].position.x = body.GetPosition().x * pixelsPerMeter;
        starInfo[i].position.y = body.GetPosition().y * pixelsPerMeter;
    }

    for (let i = 0; i < bombBodies.length; i++) {
        const body = bombBodies[i];
        bombInfo[i].position.x = body.GetPosition().x * pixelsPerMeter;
        bombInfo[i].position.y = body.GetPosition().y * pixelsPerMeter;
    }
